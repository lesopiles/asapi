...
def get_cars():
    """
    Получение списка автомобилей с возможностью фильтрации и пагинации.

    :query brand: Марка автомобиля (опционально)
    :query model: Модель автомобиля (опционально)
    :query gen: Поколение модели (опционально)
    :query page_num: Номер страницы (по умолчанию '1')
    :query year_from: Год выпуска от (опционально)
    :query year_to: Год выпуска до (опционально)
    :query price_from: Цена от (опционально)
    :query price_to: Цена до (опционально)

    :return: JSON с данными автомобилей и информацией о пагинации
    :rtype: flask.Response

    :Example HTTP GET:
        GET /api/v1/cars?brand=Toyota&model=Camry&page_num=2
    ...
    def task():
        ...
        try:
            ...
            cars_data = scraper.scrape_cars(page_num, filters)
...
@app.route("/api/v1/filters/models", methods=["GET"])
...
def get_brand_models():
    ...
    :Example HTTP GET:
        GET /api/v1/filters/models?brand=Toyota
...
@app.route("/api/v1/filters/gens", methods=["GET"])
...
def get_model_gens():
    ...
    :Example HTTP GET:
        GET /api/v1/filters/gens?brand=Toyota&model=Camry
